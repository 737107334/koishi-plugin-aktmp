...
function apply(ctx, cfg) {
    ...
    ctx.setInterval(async () => {
        await tmpDlcMap.refreshCache(ctx);
    }, 7200000); // 2小时
    ...
    ctx.command('今日里程排行 [tmpId]', '查看今日里程排行').action(async ({ session }, tmpId) => await tmpMileageRanking(ctx, session, MileageRankingType.today, tmpId));
    ctx.command('总里程排行 [tmpId]', '查看总里程排行').action(async ({ session }, tmpId) => await tmpMileageRanking(ctx, session, MileageRankingType.total, tmpId));
