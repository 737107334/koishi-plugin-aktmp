...
module.exports = async (ctx, cfg, session, tmpId) => {
    if (ctx.puppeteer) {
        ...
        // 查询周边玩家，并处理数据
        let areaPlayersData = await truckersMpMapApi.area(ctx.http, playerMapInfo.data.server, playerMapInfo.data.x - 4000, playerMapInfo.data.y + 2500, playerMapInfo.data.x + 4000, playerMapInfo.data.y - 2500);
        let areaPlayerList = [];
        ...
        // 处理城市名称，提取括号前的部分（与路况显示保持一致）
        let cityName = playerMapInfo.data.location.poi.realName;
        ...
        let data = {
            ...
            realName: await baiduTranslate(ctx, cfg, cityName),
